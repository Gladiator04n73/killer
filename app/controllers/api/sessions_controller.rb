class Api::SessionsController < Devise::SessionsController
  ...
  def create
    Rails.logger.info "Starting session creation process"

    if current_user
      Rails.logger.warn "User already authenticated. Current user ID: #{current_user.id}"
    ...
    if email.blank? || password.blank?
      Rails.logger.error "Missing email or password in request. Params: #{params.inspect}"
    ...
    begin
      ...
      sign_in(user)
      Rails.logger.info "User authenticated successfully: #{user.id}. Setting cookies."
  ...
  def show
    Rails.logger.info "GET /api/sessions request received"
  ...
  def destroy
    Rails.logger.info "Starting logout process for user."

    begin
      signed_out = (Devise.sign_out_all_scopes ? sign_out : sign_out(resource_name))
      if signed_out
        cookies.delete(:user_id)
        Rails.logger.info "User successfully logged out. User ID: #{cookies.signed[:user_id]}"
